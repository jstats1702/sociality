...
B <- nrow(samples$U_chain)  # Get number of samples
test_degree_distance <- matrix(NA, B, I)  # Preallocate matrix
...
for (b in seq_len(B)) {
     ...
     # Compute statistics efficiently
     test_degree_distance[b, ] <- igraph::degree(g)
...
save(test_degree_distance, file = "test_degree_distance_lazega.RData")
